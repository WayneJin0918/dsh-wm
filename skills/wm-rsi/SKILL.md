...
- Which Vision Toolkit tool runs on which worst frame.
...
3. **Measure the current world.** `wm_discover` → `wm_summarize` and/or `wm_rollout_diff`. Optional Vision Toolkit on worst frames.
