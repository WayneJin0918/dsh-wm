...
`wm_inspect` the action window after `wm_rollout_diff` has named the frames. Open `wm_view` to scrub pred vs GT with the action HUD (arrow + followed / dropped) on the same timeline.
